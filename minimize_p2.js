...
jQuery(function($) {
    WidgetsModuleInitialize();
});

function WidgetsModuleInitialize() {
    $(document).on('s123.page.ready', function(event) {
        var $section = $('section.s123-widgets-module');
        $section.each(function(index) {
            var $sectionThis = $(this);
            var $promoContainer = $sectionThis.find('.promo-container');
            var $shapeTop = $sectionThis.find('.shapeTop');
                    ...
                    submitHandler: function(form) {
                        ...
                        $.ajax({
                            ...
                            success: function(data) {
                                ...
                                $form.trigger("reset");
                                if (clickAction == 'thankYouMessage' || clickAction == '') {
                                    var structureID = $form.closest('.w-m-structure').get(0).id;
                                    var columnID = $form.closest('.w-m-column').get(0).id;
                                    var widgetID = $form.closest('.s123-widget').get(0).id;
                                    bootbox.alert({
                                        title: translations.sent,
                                        message: translations.ThankYouAfterSubmmit + '<iframe src="/versions/' + $('#versionNUM').val() + '/include/contactSentO.php?w=' + $('#w').val() + '&websiteID=' + dataObj.websiteID + '&moduleID=' + dataObj.moduleID + '&moduleTypeNUM=159&widgetID=' + widgetID + '&columnID=' + columnID + '&structureID=' + structureID + '" style="width:100%;height:30px;" frameborder="0"></iframe>',
                                        className: 'contactUsConfirm',
                                        buttons: {
                                            ok: {
                                                label: translations.Ok
            ...
            var widgetsScreenChangeHandler = new WidgetsScreenChangeHandler({
                sectionID: $sectionThis.data('module-id'),
                $section: $sectionThis,
            });
        });
    ...
    function WidgetsScreenChangeHandler(settings) {
        var _ = {
            sectionID: settings.sectionID,
            $section: settings.$section,
            $texts: {}
        ...
        _.init = function() {
            _.$section.find('.promoText').each(function(index, promoText) {
                var $promoText = $(promoText);
                _.$texts[$promoText.get(0).id] = {
                    $el: $promoText,
                    orignialFontSize: $promoText.css('font-size')
            ...
            if (IsWizard()) {
                $(document).off('wizard.preview.device.changed.WidgetsScreenChangeHandler_' + _.sectionID).on('wizard.preview.device.changed.WidgetsScreenChangeHandler_' + _.sectionID, function(event) {
                    $.each(_.$texts, function(index, $text) {
                        _.fitFontSize($text.$el);
                    });
                });
                _.$section.off('resizableText.end.WidgetsScreenChangeHandler').on('resizableText.end.WidgetsScreenChangeHandler', function(event, $el, newFontSize) {
                    _.updateOriginalFontSize($el.get(0).id, newFontSize + 'px');
                });
                _.$section.off('widgetManager.add.WidgetsScreenChangeHandler').on('widgetManager.add.WidgetsScreenChangeHandler', function(event, $el) {
                    _.refresh();
                });
            }
        };
    ...
    function GetShapeColor(position, id) {
        var sectionPlace = $('section#' + id).get(0).style.order;
        var $websiteSections = $('#s123ModulesContainer section');
        if (sectionPlace.length == 0) {
            $websiteSections.each(function(index, value) {
                if ($(value).attr('id') == id) {
                    sectionPlace = index;
